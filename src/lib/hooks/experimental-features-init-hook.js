...
function toggleExperimentalFeatures(type, hiddenFeatures) {
    ...
    if (hiddenFeatures.length > 0) {
        let filtered = this.config[type]
            .filter(e => !hiddenFeatures.find(h => e[key].startsWith(h)));
        let removedCount = this.config[type].length - filtered.length;
        if (removedCount !== 0) {
            this.config[`_${type}`] = filtered
                .reduce((acc, e) => {
                    acc.set(e[key], e);
                    return acc
                }, new Map());
...
module.exports = async function() {
    let plugin = this.config.plugins.find(p => p.name === '@adobe/aio-cli-plugin-aem-rde');
    if (plugin) {
        const experimentalFeatures = plugin.pjson.oclif['experimental-features'] || [];
