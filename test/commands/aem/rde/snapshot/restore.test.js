...
describe('RestoreSnapshot', function () {
  describe('#runCommand', function () {
    ...
    const stubbedRestoreResponseFailure = (status = 400, details = '') => ({
      status,
      json: async () => ({
        actionid: 1234123,
        success: false,
    ...
    const prepareStubs = (cloudSdkMethods) => {
      command = new RestoreSnapshot([], {}, 10);
    ...
    it('works', async function () {
      ...
      assert.equal(cloudSdkApiStub.restoreSnapshot.calledOnce, true);
      ...
      expect(addSpinniesSpy.getCall(2).args[0]).to.equal('spinner-restore');
      ...
      verifySpinnySucceeded(
        ...
        (obj) =>
          obj.text.startsWith(
            'Requested to restore the snapshot successfully.'
      ...
      verifySpinnySucceeded(
        ...
        (obj) =>
          obj.text.startsWith(
            'Backend picked up the job to restore the snapshot.'
          ) && obj.successColor === 'greenBright'
      );
      verifySpinnySucceeded(
        2,
        'spinner-restore',
        (obj) =>
          obj.text.startsWith('Restored snapshot to RDE successfully.') &&
          obj.successColor === 'greenBright'
      );
    });
    ...
    it('catches a bad progress result (-2)', async function () {
      ...
      try {
        ...
        expect(err.message).to.contain('The snapshot failed to be restored.');
      }
    });
    ...
    it('catches a withCloudSdk exception - createSnapshot', async function () {
      prepareStubs({
        restoreSnapshot: stubReject('Internal error'),
      });
    ...
    it('calls the appropriate api and shows failures', async function () {
      const checkError = async (statusCode, statusDetails, expectedMessage) => {
        prepareStubs({
          restoreSnapshot: stub(
            stubbedRestoreResponseFailure(statusCode, statusDetails)
          ),
          getSnapshotProgress: stub(getSnapshotProgressResponse()),
          getArtifacts: stub(getArtifactsResponse),
        });
      ...
      await checkError(
        404,
        'The snapshot is in deleted state.',
