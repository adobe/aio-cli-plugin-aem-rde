...
const { configInputBuild } = require('../../src/lib/config.js');
...
const config = proxyquire('../../src/lib/config.js', {
  fs: {
...
const archConfig = proxyquire('../../src/lib/config.js', {
  fs: {
...
describe('Archive Utility', function () {
  ...
  beforeEach(async function () {
    ...
    tmpDir = await fs.mkdtempSync(
      path.join(os.tmpdir(), 'aio-rde-config-test-')
    );
  });
  ...
  describe('#archiveDirectory', function () {
    it('should call archiver functions', async function () {
      ...
      const { inputPath, inputPathSize } = await configInputBuild(
        command,
        tmpDir
      );
    ...
    it('should add directory to archive', async function () {
      ...
      sinon.stub(command, 'doLog');
      try {
        await archConfig.configInputBuild(command, '');
      } catch (e) {
        err = e;
      }
  ...
  describe('#configInputBuild', function () {
